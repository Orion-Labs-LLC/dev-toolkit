...
describe("Node.js ESLint Configuration", () => {
  ...
  it("should include base configuration", () => {
    const tsConfig = nodeConfig.find(
  ...
  it("should configure Node.js globals", () => {
    const nodeGlobalsConfig = nodeConfig.find(
      (config: TSESLint.FlatConfig.Config) => config.languageOptions?.globals,
    );
  ...
  it("should disable DOM-specific unicorn rules", () => {
    const nodeSpecificConfig = nodeConfig.find(
      (config: TSESLint.FlatConfig.Config) =>
