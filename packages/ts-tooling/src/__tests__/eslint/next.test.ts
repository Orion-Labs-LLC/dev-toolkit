...
describe('Next.js ESLint Configuration', () => {
  ...
  it('should include React configuration', () => {
    ...
    const reactPluginConfig = nextConfig.find((config: TSESLint.FlatConfig.Config) => 
  ...
  it('should configure Next.js plugin', () => {
    const nextPluginConfig = nextConfig.find((config: TSESLint.FlatConfig.Config) => 
  ...
  it('should include Next.js recommended and core-web-vitals rules', () => {
    const nextPluginConfig = nextConfig.find((config: TSESLint.FlatConfig.Config) => 
      config.plugins?.['@next/next'] && config.rules
    );
  ...
  it('should inherit base and React configurations', () => {
    ...
    const tsConfig = nextConfig.find((config: TSESLint.FlatConfig.Config) => 
      config.rules?.['@typescript-eslint/consistent-type-imports']
    );
    expect(tsConfig).toBeDefined();

    // Should include React rules
    const reactRules = nextConfig.find((config: TSESLint.FlatConfig.Config) => 
      config.rules?.['react/react-in-jsx-scope']
    );
