...
describe("React ESLint Configuration", () => {
  ...
  it("should include base configuration", () => {
    ...
    const tsConfig = reactConfig.find(
  ...
  it("should configure React plugins", () => {
    const reactPluginConfig = reactConfig.find(
  ...
  it("should disable React in JSX scope rule", () => {
    const reactPluginConfig = reactConfig.find(
      (config: TSESLint.FlatConfig.Config) => config.plugins?.react && config.rules,
    );
  ...
  it("should configure React version detection", () => {
    const reactPluginConfig = reactConfig.find(
      (config: TSESLint.FlatConfig.Config) => config.settings?.react,
    );
    expect(reactPluginConfig).toBeDefined();
  ...
  it("should configure Tailwind CSS plugin", () => {
    const tailwindConfig = reactConfig.find(
      (config: TSESLint.FlatConfig.Config) => config.plugins?.tailwindcss,
    );
  ...
  it("should configure Tailwind CSS callees", () => {
    const tailwindConfig = reactConfig.find(
      (config: TSESLint.FlatConfig.Config) => config.settings?.tailwindcss,
    );
